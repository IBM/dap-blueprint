...
def CKD_priv(parent_privkey: bytes, parent_chaincode: bytes, child_index: int) -> Tuple[bytes, bytes]:
    ...
    """
    print('bip32:CKD_priv parent_privkey={} child_index={}'.format(parent_privkey.hex(), child_index))
...
def _CKD_priv(parent_privkey: bytes, parent_chaincode: bytes,
    ...
    print('bip32:_CKD_priv: child_index={} is_hardened_child={} parent_privkey={} child_privkey={}'.format(child_index.hex(), is_hardened_child, parent_privkey.hex(), child_privkey.hex()))
...
def _CKD_pub(parent_pubkey: bytes, parent_chaincode: bytes, child_index: bytes) -> Tuple[bytes, bytes]:
    print('bip32:_CKD_pub: parent_pubkey={} child_index={}'.format(parent_pubkey.hex(), child_index))
    ...
    child_chaincode = I[32:]
    print('bip32:_CKD_pub: child_index={} parent_pubkey={} child_pubkey={}'.format(child_index.hex(), parent_pubkey.hex(), child_pubkey.hex()))
...
class BIP32Node(NamedTuple):
    ...
    def from_xkey(cls, xkey: str, *, net=None) -> 'BIP32Node':
        if net is None:
            net = constants.net
        print('bip32:BIP32Node:from_xkey xkey={}'.format(xkey))
        xkey = DecodeBase58Check(xkey)
        print('bip32:BIP32Node:from_xkey xkey={}'.format(xkey.hex()))
        if len(xkey) != 78:
        ...
        print('bip32:BIP32Node:from_xkey: xtype={} depth={} child_number={} pubkey={} fingerprint={} is_private={}'.format(xtype, depth, child_number.hex(), eckey.get_public_key_hex(), fingerprint.hex(), is_private))
    ...
    # Initial entry to create a BIP32Node
    @classmethod
    def from_rootseed(cls, seed: bytes, *, xtype: str) -> 'BIP32Node':
        I = hmac_oneshot(b"Bitcoin seed", seed, hashlib.sha512)
        master_k = I[0:32]
        master_c = I[32:]
        print('bip32:BIP32Node:from_rootseed: master_k={} master_c={} eckey={} pubkey={} xtype={}'.format(master_k.hex(), master_c.hex(), ecc.ECPrivkey(master_k), ecc.ECPrivkey(master_k).get_public_key_hex(), xtype))
        return BIP32Node(xtype=xtype,
    ...
    def from_bytes(cls, b: bytes) -> 'BIP32Node':
        print('bip32:BIP32Node:from_bytes: bytes={}'.format(b))
        if len(b) != 78:
    ...
    def to_bytes(self, *, net=None) -> bytes:
        print('bip32:BIP32Node:to_bytes')
        if self.is_private():
    ...
    def convert_to_public(self) -> 'BIP32Node':
        print('bip32:BIP32Node:convert_to_public')
        if not self.is_private():
            return self
    ...
    def subkey_at_private_derivation(self, path: Union[str, Iterable[int]]) -> 'BIP32Node':
        ...
        if not path:
            print('bip32:IP32Node:subkey_at_private_derivation: privkey={} pubkey={} fingerprint={} path={} self={}'.format(self.eckey.get_secret_bytes().hex(), self.eckey.get_public_key_hex(), hash_160(self.eckey.get_public_key_bytes(compressed=True))[0:4].hex(), pathstr, self))
            return self
        depth = self.depth
        chaincode = self.chaincode
        privkey = self.eckey.get_secret_bytes()
        ss = [str(p) for p in path]
        print('bip32:IP32Node:subkey_at_private_derivation: parent_privkey={} parent_pubkey={} parent_fingerprint={} path={} depth={}'.format(privkey.hex(), self.eckey.get_public_key_hex(), hash_160(self.eckey.get_public_key_bytes(compressed=True))[0:4].hex(), '/'.join(ss), depth))
        ...
        print('bip32:BIP32Node:subkey_at_private_derivation: xtype={} depth={} child_number={} privkey={} pubkey={} fingerprint={} parent_privkey={} parent_pubkey={} parent_fingerprint={} path={}'.format(self.xtype, depth, child_number.hex(), privkey.hex(), pubkey.hex(), hash_160(pubkey)[0:4].hex(), parent_privkey.hex(), parent_pubkey.hex(), fingerprint.hex(), '/'.join([str(int) for int in path])))
    ...
    def subkey_at_public_derivation(self, path: Union[str, Iterable[int]]) -> 'BIP32Node':
        print('bip32:BIP32Node:subkey_at_public_derivation: path={}'.format(path))
        if path is None:
            raise Exception("derivation path must not be None")
        pathstr = path
        if isinstance(path, str):
            path = convert_bip32_path_to_list_of_uint32(path)
        if not path:
            print('bip32:IP32Node:subkey_at_public_derivation: pubkey={} fingerprint={} path={} self={}'.format(self.eckey.get_public_key_hex, hash_160(self.eckey.get_public_key_bytes(compressed=True))[0:4].hex(), pathstr, self))
            return self.convert_to_public()
        depth = self.depth
        chaincode = self.chaincode
        pubkey = self.eckey.get_public_key_bytes(compressed=True)
        ss = [str(p) for p in path]
        #print('bip32:subkey_at_public_derivation: path={} depth={}'.format('/'.join([str(int) for int in path]), depth))
        print('bip32:IP32Node:subkey_at_public_derivation: parent_pubkey={} parent_fingerprint={} path={} depth={}'.format(self.eckey.get_public_key_hex, hash_160(self.eckey.get_public_key_bytes(compressed=True))[0:4].hex(), '/'.join(ss), depth))
        for child_index in path:
        ...
        fingerprint = hash_160(parent_pubkey)[0:4]
        child_number = child_index.to_bytes(length=4, byteorder="big")
        #print('bip32:BIP32Node:subkey_at_public_derivation: xtype={} depth={} child_number={} fingerprint={} parent_pubkey={} path={}'.format(self.xtype, depth, child_number.hex(), fingerprint.hex(), parent_pubkey.hex(), '/'.join([str(int) for int in path])))
        print('bip32:BIP32Node:subkey_at_public_derivation: xtype={} depth={} child_number={} pubkey={} fingerprint={} parent_pubkey={} parent_fingerprint={} path={}'.format(self.xtype, depth, child_number.hex(), pubkey.hex(), hash_160(pubkey)[0:4].hex(), parent_pubkey.hex(), fingerprint.hex(), '/'.join([str(int) for int in path])))
        return BIP32Node(xtype=self.xtype,
    ...
    def calc_fingerprint_of_this_node(self) -> bytes:
        ...
        """
        pubkey = self.eckey.get_public_key_bytes(compressed=True)
        print('bip32:BIP32Node:calc_fingerprint_of_this_node pubkey={} fingerprint={}'.format(pubkey.hex(), hash_160(pubkey)[0:4].hex()))
...
def convert_bip32_path_to_list_of_uint32(n: str) -> List[int]:
    ...
    """
    print('bip32:convert_bip32_path_to_list_of_unit32 n={}'.format(n))
    if not n:
        return []
    if n.endswith("/"):
        n = n[:-1]
    ...
    print('bip32:convert_bip32_path_to_list_of_unit32 path={}'.format(path))
    return path
...
def root_fp_and_der_prefix_from_xkey(xkey: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    print('root_fp_and_der_prefix_from_xkey depth={} derivation_prefix={} root_fingerprint={} pubkey={} pubkey_fingerprint={} node={}'.format(node.depth, derivation_prefix, root_fingerprint, node.eckey.get_public_key_hex(), node.calc_fingerprint_of_this_node().hex(), node))
