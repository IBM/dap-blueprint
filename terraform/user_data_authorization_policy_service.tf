...
locals {
  ...
  authorization_policy_service_contract_plain = yamlencode({
    "env" : yamlencode(local.env),
    "workload" : yamlencode(local.authorization_policy_service_workload)
  })
}
...
resource "local_file" "authorization_policy_service_contract" {
  content  = local.authorization_policy_service_contract_plain
