...
locals {
  ...
  signing_service_contract_plain = yamlencode({
    "env" : yamlencode(local.env),
    "workload" : yamlencode(local.signing_service_workload)
  })
}
...
resource "local_file" "signing_service_contract" {
  content  = local.signing_service_contract_plain
  filename = "ss_plain.yml"
}
